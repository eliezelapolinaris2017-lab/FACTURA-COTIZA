...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------- PDF A4 (1 sola página con auto-ajuste y tope de 15 líneas) ----------
  $("#btnPrint")?.addEventListener("click", printDoc);

  function printDoc(){
    // Limita a 15 filas para mantener 1 página limpia
    const rows = [...document.querySelectorAll("#linesBody tr")];
    if (rows.length > 15) {
      alert("Para un PDF de una sola página, usa máximo 15 líneas. Elimina o guarda en dos documentos.");
      return;
    }
    ...
    sheet.classList.remove("compress-1","compress-2","compress-3");

    // Conversión px↔mm para medir altura real
    const probe = document.createElement("div");
    probe.style.width="100mm"; probe.style.height="100mm"; probe.style.position="absolute"; probe.style.left="-9999px";
    document.body.appendChild(probe);
    const pxPerMM = probe.getBoundingClientRect().width / 100;
    document.body.removeChild(probe);

    // Altura útil A4 = 297 - (16 + 16) = 265 mm
    const usableMM = 297 - 16 - 16;
    const getSheetMM = ()=> sheet.getBoundingClientRect().height / pxPerMM;

    let level=0;
    while (getSheetMM() > usableMM && level < 3){
      level++;
      sheet.classList.add(`compress-${level}`);
    }
